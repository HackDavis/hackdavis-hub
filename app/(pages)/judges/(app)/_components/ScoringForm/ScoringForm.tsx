...
export default function ScoringForm({ team, submission }: ScoringFormProps) {
  const categorizedTrackNames = Object.keys(judgeVisibleTracks);
  ...
  const unfilledDynamicQuestions = Object.fromEntries(
    scorableTracks
      .map((trackName) =>
        (judgeVisibleTracks[trackName].scoring_criteria ?? []).map((track) => [
    ...
    <form onSubmit={onSubmit} className={styles.form}>
      ...
      {scorableTracks.map((category) => (
        <div key={category} className={styles.track_container}>
          ...
          {(judgeVisibleTracks[category].scoring_criteria ?? []).map(
            (question) => (
              <RadioSelect
