...
export default async function GenerateInvite(role: string, data: InviteData) {
  try {
    if (role !== 'hacker' && role !== 'judge') {
      throw new HttpError('Unknown role. Choose either hacker or judge.');
    }

    const path = role === 'judge' ? '/judges' : '';
    data['exp'] =
      Date.now() +
      1000 *
        60 *
    ...
    const hmac_url = `${process.env.BASE_URL}${path}/invite/${data_encoded}&${hmac_sig}`;
